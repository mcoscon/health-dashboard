...
import { getPatientGraphData, setPatientGraphLoading } from '../store/patientGraphSlice'
...
const PatientGraphs = () => {
	const dispatch = useDispatch()
	const isLoading = useSelector((state) => state.patientGraphState.loading)
	useEffect(() => {
		setTimeout(() => {
			dispatch(getPatientGraphData())
		}, 1200)
	}, [dispatch])
			...
			<div className='p-4 w-[85vw] sm:w-[65vw] md:w-[50vw] flex flex-col '>
				<div className='flex flex-row items-center justify-between gap-4'>
					...
					<div className='flex items-center gap-2'>
						...
						<button
							className='flex flex-row items-center justify-center gap-2 p-2 text-xs text-white transition duration-150 rounded-lg shadow-btnShadow font-Karla-Heavy bg-cyan-700 hover:bg-cyan-600'
							onClick={() => {
								dispatch(setPatientGraphLoading())
								setTimeout(() => {
									console.log('run')
									dispatch(getPatientGraphData())
								}, 1200)
